...
For the full pattern decision tree with verification steps, see [00_url_setup_prompt.md](00_url_setup_prompt.md). Below is a quick reference linking each pattern to a real, tested implementation.

| # | Pattern | Use When | Reference Implementation |
|---|---------|----------|--------------------------|
| 1 | Static HTML | Plain HTML, no JS needed | `rules/cities/langenfeld/city_events/` |
| 2 | Dynamic JS + Load More | Page requires clicking "Load More" | `rules/cities/monheim_am_rhein/terminkalender/` |
| 3 | Multiple Regex Patterns | Multiple event formats on same page | `rules/cities/leverkusen/stadt_erleben/regex.py` |
| 4 | Date Filter Parameters | URL accepts date range query params | `rules/cities/leverkusen/stadt_erleben/scraper.py` |
| 5 | All-Day Events | Events without specific times | Any `_create_event_from_match()` — use `time=""` |
| 6 | Pagination | Multiple pages of results | `rules/cities/hitdorf/kalender/scraper.py` |
| 7 | REST API (JSON) | Site provides a JSON API | `rules/cities/leverkusen/lust_auf/scraper.py` |
| 8 | HTML Parsing (BeautifulSoup) | Structured HTML easier than regex | `rules/cities/monheim_am_rhein/terminkalender/regex.py` |
| 9 | Disable Level 2 | Detail pages not needed or too slow | `rules/cities/dormagen/feste_veranstaltungen/scraper.py` (set `DISABLE_LEVEL_2 = True`)

**Key implementation notes:**
- **REST API scrapers** override `fetch_events_from_api()` and return `Event` objects directly (bypasses regex)
- **HTML parsers** override `parse_with_regex()` in the regex class and return `Event` objects
- **Level 2 disabled** via `DISABLE_LEVEL_2 = True` class attribute on the scraper
...
ls -lt logs/ | head -2
```
Log files contain event counts, errors, Level 2 progress, and final status. Bash timeouts do not mean script failure — the script continues running in the background.
